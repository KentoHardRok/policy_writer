...
df = pd.read_csv('/config/aws_address.csv')
...
dfprint['name'].to_csv('aws_name_only.csv', index=False)

# open template
with open('addobj_xml.j2') as file:
    template = Template(file.read())

with open('config/aws_addobj.xml', 'a') as f:
    f.write('<config>\n')
    f.write('  <shared>\n')
    f.write('    <address>\n')
