...
# Check the length of each value in the "name" column
name_mask = df['name'].str.len() > 30

# Copy the values to the "comment" column and keep only the last 30 characters
df.loc[name_mask, 'comment'] = df.loc[name_mask, 'name']

# Truncate the "name" column to the last 30 characters for the selected rows
df.loc[name_mask, 'name'] = df.loc[name_mask, 'name'].str[-30:]

# now I eliminate most special characters
