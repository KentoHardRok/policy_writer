...
dfpol = pd.read_csv('va3_pol.csv')
dfadd = pd.read_csv('va3_name_only.csv')
dfgrp = pd.read_csv('va3_grpname_only.csv')
dfsrv = pd.read_csv('va3_srvname_only.csv')
dfzone = pd.read_csv('va3_zone.csv')
dfqdn = pd.read_csv('va3_customurlname_only.csv')
...
dfdev_2 = dfdev.get_group('VA3_Edge_Vsys')
...
with open('config/va3_secpol_fail_desc.xml', 'a') as f:
    f.write('<config>\n')
    f.write('  <devices>\n')
    f.write('    <entry name="localhost.localdomain">\n')
    f.write('      <device-group>\n')
    for i, df in enumerate([dfdev_1, dfdev_2, dfdev_3, dfdev_4], start=1):
        ...
        # per row
                secpol_config = template.render(
                    ...
                    virus=row['av-profile'],
                    urlfiltering=row['webfilter-profile'],
                    vulnerability=row['ips-sensor'],
                    rule_name=row['name'],
                    srcip=row['srcaddr'],
                    dstip=row['dstaddr'],
                    userid=row['groups'],
